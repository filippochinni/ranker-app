...
class PrintHandler:
	...
	@staticmethod
	def print_written_status(output_file, was_written):
		if was_written:
			print(f"Table written to {output_file}")
		else:
			print(f"Nothing written to {output_file}")

	def print_status_updated(self, input_file, table):
		print(f"Computing table for {input_file}...")
		print(f"\nComputed Table:\n\n{table}\n")

	def print_status_unchanged(self, output_file, table):
		print(f"\n{os.path.basename(output_file)} has not changed, there is no need to update the file")
		print(f"\nComputed Table:\n\n{table}\n")
